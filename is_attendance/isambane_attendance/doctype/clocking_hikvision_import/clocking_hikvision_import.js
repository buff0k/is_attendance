...
const IS_ATTENDANCE_HIK_POLL_INTERVAL_MS = 5000;
const IS_ATTENDANCE_HIK_POLL_MAX_TICKS = 360; // ~30 minutes of polling before giving up

frappe.ui.form.on("Clocking HIKVision Import", {
	refresh(frm) {
		is_attendance.clocking_hikvision_import.stop_polling(frm);
		...
		is_attendance.clocking_hikvision_import.render_status(frm);

		if (frm.doc.status === "Importing") {
			is_attendance.clocking_hikvision_import.start_polling(frm);
		}
	},
});

frappe.provide("is_attendance.clocking_hikvision_import");

is_attendance.clocking_hikvision_import.render_status = function (frm) {
...
is_attendance.clocking_hikvision_import.start_polling = function (frm) {
	let ticks = 0;
	...
	frm._is_attendance_poll = setInterval(() => {
		...
		if (ticks > IS_ATTENDANCE_HIK_POLL_MAX_TICKS || cur_frm !== frm || frm.docname !== docname) {
			is_attendance.clocking_hikvision_import.stop_polling(frm);
			return;
		}

		frappe.db.get_value("Clocking HIKVision Import", docname, ["status", "docstatus"]).then((r) => {
			const values = r.message || {};
			if (values.status !== frm.doc.status || values.docstatus !== frm.doc.docstatus) {
				frm.reload_doc();
			}
		});
	}, IS_ATTENDANCE_HIK_POLL_INTERVAL_MS);
};

is_attendance.clocking_hikvision_import.stop_polling = function (frm) {
